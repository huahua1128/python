...
from zgh.unittest_test.zuoye_0105.peizhi_file import ReadConfig

class MyLog:
    '''自己写的日之类'''

    def __init__(self):
        self.in_level = ReadConfig('math_method.conf').get_value('LOG', 'in_level')
        self.out_level = ReadConfig('math_method.conf').get_value('LOG', 'out_level')
        self.file_level = ReadConfig('math_method.conf').get_value('LOG', 'file_level')
        self.file_path = ReadConfig('math_method.conf').get_value('LOG', 'file_path')
        self.format = ReadConfig('math_method.conf').get_value('LOG', 'formatter')
        # 收集器 -----创建一个日志收集器
