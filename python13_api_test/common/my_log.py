...
class MyLog:
    ...
    def __init__(self):
        ...
        # 收集器 -----创建一个日志收集器
        self.my_logger = logging.getLogger('python')  # getLogger 是个函数，必须传一个参数，作为自己的日志收集器的名字，否则还是会用root logger
        self.my_logger.setLevel(self.in_level)  # 给日志收集器设置level   (相当于第一次过滤)  设置了日志收集器的level,上面加不加名字都可以。但是一般还是要加上名字
